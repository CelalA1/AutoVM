...
class Server extends \yii\db\ActiveRecord
{
    ...
    {
        return [
            self::SCENARIO_DEFAULT => ['parent_id', 'name', 'ip', 'port', 'username', 'password', 'vcenter_ip', 'vcenter_username', 'vcenter_password', 'network', 'second_network', 'version', 'virtualization', 'dns1', 'dns2', 'rsa', 'server_address'],
        ];
    }
    ...
    public function afterSave($insert, $changedAttributes)
    {
        ...
        if ($server && $insert) {
            ...
            if ($server->virtualization == 'KVM') {
                
                $path = Yii::getAlias('@app/models/upload.py');
                              
                if ($this->rsa) {
                    shell_exec("python2.7 $path --address={$server->ip} --username={$server->username} --password={$server->password} --content=\"{$this->rsa}\"");
                }
